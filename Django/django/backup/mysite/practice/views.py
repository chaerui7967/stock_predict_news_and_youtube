...
def news_pop(st_cd,dates):
    ...
    for cd in st_cd:
        ...
        if result == 0:
            ...
            query2 = 'SELECT * FROM accuracy_point where st_nm = "{}" and datetime like "{}%" order by ratio desc limit 1;'.format(
                cd, dates)
...
def analysis_result(st_n):
    ...
    row = {
        ...
        'close':str(analysis_result[13].values[0])
    }
...
def samsung(request,yearid=0):
    ...
    return render(request, 'samsung.html', {'finacedataJSON': finacedataJSON,'datefinacedataJSON':datefinacedataJSON,
                                            'newsJSON':newsJSON,'wordcloudJSON':wordcloudJSON,
