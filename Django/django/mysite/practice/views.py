...
def news_pop(st_cd,dates):
    ...
    for cd in st_cd:
        ## accuracy
        # query2 = 'SELECT * FROM accuracy_point where st_nm = "{}" and datetime like "{}%" order by ratio desc limit 1;'.format(cd,dates)
        #query2 = 'SELECT * FROM accuracy_point where st_nm = "{}" order by datetime desc,ratio desc limit 1;'.format(cd)
        query2 = 'SELECT * FROM accuracy_point_2 where st_nm = "{}" order by abs(score) desc,datetime desc limit 1;'.format(cd)
        result = cursor.execute(query2)
        if result == 0:
    ...
    for i in range(len(result_re)):
        row = {
            ...
            'Ratio':str(round(result_re.iloc[i][7],2))
        }
        finacedata.append(row)
    newsJSON = json.dumps(finacedata)
    return newsJSON
...
def news_pop_neg(st_nm,dates):
    ...
    # query2 = 'SELECT * FROM accuracy_point where st_nm = "{}" and datetime like "{}%" and news="{}" and abs(Ratio-0.5) not in(0.5,0) order by abs(Ratio-0.5) desc limit 1;'.format(st_nm,'20210928','매일경제')
...
#        st_nm,'매일경제')
    query2 = 'SELECT * FROM accuracy_point_2 where st_nm="{}" and news="{}" and abs(score) not in (0) order by abs(score) desc,  datetime desc limit 1;'.format(st_nm,'매일경제')
    result = cursor.execute(query2)
    globals()['accuracy_{}'.format(st_nm)] = pd.DataFrame(cursor.fetchall())
    result_re = result_re.append(globals()['accuracy_{}'.format(st_nm)])

    # query3 = 'SELECT * FROM accuracy_point where st_nm = "{}" and datetime like "{}%" and news="{}" and abs(Ratio-0.5) not in(0.5,0) order by abs(Ratio-0.5) desc limit 1;'.format(st_nm, '20210928','아시아경제')
    ...
    #    st_nm, '아시아경제')
    query3 = 'SELECT * FROM accuracy_point_2 where st_nm="{}" and news="{}" and abs(score) not in (0) order by abs(score) desc,  datetime desc limit 1;'.format(st_nm,'아시아경제')
...
def finance_data(st_cd,yearid=0):
    ...
    if yearid ==0:
        ...
        #query2 = 'SELECT * FROM `financedata` where st_cd = "{}" order by date limit 1;'.format(st_cd,yearid)
        result = cursor.execute(query2)
        result = pd.DataFrame(cursor.fetchall(), columns=col)
    connection.commit()
    connection.close()
...
def wordcloud_maker(request,st_cd,dates):
    ...
    if st_cd == '0':
        #query2 = 'SELECT * FROM youtube_token where datetime like "{}%";'.format(dates)
        #query2 = 'SELECT * FROM youtube_token order by datetime limit 1;'.format(dates)
        query2 = 'SELECT * FROM youtube_token_2 order by datetime limit 1;'.format(dates)
        result = cursor.execute(query2)
        result = pd.DataFrame(cursor.fetchall())
        ...
        #query2 = "SELECT * FROM youtube_token where st_cd = {} and datetime like '%{}%';".format(st_cd,dates)
        #query2 = "SELECT * FROM youtube_token where st_cd = {} order by datetime desc limit 1;".format(st_cd)
        query2 = "SELECT * FROM youtube_token_2 where st_cd = {} order by datetime desc limit 1;".format(st_cd)
        result = cursor.execute(query2)
        result = pd.DataFrame(cursor.fetchall())
    connection.commit()
    connection.close()
    ...
    #if li in
    #word_x=['부분','아마','때문','수준','이제','의미','수가','보시','정도','저희','이기','관련','워낙','최근','센티','이상','라든지','내용','음악','이야기','부터','한번','종이','일단','이구','여러','인지','왜냐면','딜도','팬츠','얘기','통해','']:
    ...
    #for key, value in list(enumerate(tags.items())):
    #    if key in ['부분','아마','때문','수준','이제','의미','수가','보시','정도','저희','이기','관련','워낙','최근','센티','이상',
    #              '라든지','내용','음악','이야기','부터','한번','종이','일단','이구','여러','인지','왜냐면','딜도','팬츠','얘기','통해','',]:
...
def index_accuracy(st_nm,dates):
    ...
    finacedata = []
    #dates='2021-09-15'
    #query2 = "SELECT date,mail_news,asia_news,sampro_youtube, hk_youtube, suka_youtube FROM analysis_result where date like '{}%' group by date;".format(dates)
...
def analysis_result(st_n):
    cursor = connection.cursor()
    # st_cd='5380'
    #analysis_result = "SELECT * FROM analysis_result where code like '%{}%' order by date desc limit 1;".format(st_n)
    analysis_result ="SELECT bb.*, f.close FROM analysis_result as bb \
    left join financedata as f on bb.code = f.st_cd and bb.date = DATE_ADD(f.Date, INTERVAL 1 DAY)\
